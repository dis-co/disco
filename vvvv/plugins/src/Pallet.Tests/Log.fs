...
module Log =
  ...
  let log_get_at_index =
    testCase "When I get an entry by index, it should be equal" <| fun _ ->
      ...
      let log =
        Log.empty
        |> Log.append (LogEntry(id1, 0u, 1u, (), None))
        |> Log.append (LogEntry(id2, 0u, 1u, (), None))
        |> Log.append (LogEntry(id3, 0u, 1u, (), None))
      ...
      Assert.Equal("Should find none at invalid index", None, Log.at 8u log)
  ...
  let log_resFold_short_circuit_test =
    testCase "Should short-circuit when folder fails" <| fun _ ->
      let log =
        Log.empty
        |> Log.append (Log.make 1u 1)
        |> Log.append (Log.make 1u 2)
        |> Log.append (Log.make 1u 3)
  ...
  let log_concat_length_test =
    testCase "Should have correct length" <| fun _ ->
      let log =
        Log.empty
        |> Log.append (Log.make 1u "four")
        |> Log.append (Log.make 1u "five")
        |> Log.append (Log.make 1u "six")

      log
      |> Log.append (Log.make 1u "one")
      |> Log.append (Log.make 1u "two")
      |> Log.append (Log.make 1u "three")
  ...
  let log_get_entries_until_test =
    testCase "Get all entries until (and including) a given index" <| fun _ ->
      ...
      |> Log.append (Log.make 1u "six")
      |> Log.until 4u
      |> assume "Should have 3 logs" 3u (Option.get >> Log.depth)
  ...
  let log_concat_ensure_no_duplicate_but_unique_entries =
    testCase "concat ensure no duplicate but unique entries" <| fun _ ->
      ...
      |> assume "Should have length 3" 3u Log.size
  ...
  let log_snapshot_remembers_last_state =
    testCase "snapshot remembers last state" <| fun _ ->
      ...
      let nodes =
        [ for n in 0u .. 5u do
            yield Node.create n ("Client " + string n) ]
        |> Array.ofList
  ...
  let log_untilExcluding_should_return_expected_enries =
    testCase "untilExcluding should return expected enries" <| fun _ ->
      ...
      |> assume "Should pick correct item"   16u        (Log.untilExcluding 15u >> Option.get >> Log.last >> Log.entryIndex)
      |> assume "Should have correct index" (Some "16") (Log.untilExcluding 15u >> Option.get >> Log.last >> Log.data)
      |> assume "Should have correct index" (Some "15") (Log.until 15u >> Option.get >> Log.last >> Log.data)      
      |> ignore

  let log_append_should_work_with_snapshots_too =
    testCase "append should work with snapshots too" <| fun _ ->
      let log = Log.empty |> Log.append (Snapshot(Guid.NewGuid(), 0u, 0u, 9u, 1u, Array.empty, "hello"))
      expect "Log should be size 1" 1u Log.size log
  ...
  let log_firstIndex_should_return_correct_results =
    testCase "firstIndex should return correct results" <| fun _ ->
      ...
      let indices, log =
        ...
        let def = LogEntry(System.Guid.NewGuid(),0u,0u,(),None)
        ...
        [ for term in 1u .. 4u do
            ...
            for index in uint32(offset) .. uint32(offset + random.Next(10,70)) do
              let (_,t,i) as result = (System.Guid.NewGuid(), term, index)
  ...
  let log_getn_should_return_right_number_of_entries =
    testCase "getn should return right number of entries" <| fun _ ->
      ...
      expect "should have correct depth" (uint32 n) Log.size log

      let get = System.Random().Next(1,n - 1) |> uint32
