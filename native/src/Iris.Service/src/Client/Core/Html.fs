...
module Html = 
  ...
  *)
  ...
  [<Direct("""return new virtualDom.VNode($tag, $prop, $children)""")>]
  let mkVNode (tag : string) (prop : VProperties) (children : VTree array) : VTree =
    failwith "never"
  
  [<Direct("""return new virtualDom.VText($txt)""")>]
  let mkVText (txt : string) : VTree =
    failwith "never"
  
  [<Direct("""return virtualDom.create($tree)""")>]
  let createElement (tree : VTree) : Dom.Element =
    failwith "never"
  
  [<Direct("""return virtualDom.diff($tree, $newtree)""")>]
  let diff (tree : VTree) (newtree : VTree) : VPatch =
    failwith "never"
  
  [<Direct("""return virtualDom.patch($root, $patch)""")>]
  let patch (root : Dom.Element) (patch : VPatch) : Dom.Element =
    failwith "never"
  
  [<Direct("""return new Object()""")>]
  ...
  [<Direct("""
           ...
           return $props;
           """)>]
  let addAttr (props : VProperties) (prop : string) (value : obj) : VProperties =
    failwith "never"
  
  
  [<Direct(""" return "yay!"; """)>]
  let runtimeFunName (f : obj) : string = failwith "never"
