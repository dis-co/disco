module Disco.Web.Core.MockData
...
let tags = [| "raft"; "disco"; "remote"; "git"; "store"; "persistence"; "frontend"; "yaml" |]
...
let makeClient service (name: Name) : DiscoClient =
  { Id = DiscoId.Create()
    Name = name
    Role = Role.Renderer
    ServiceId = service
...
let machines =
  List.map (fun idx ->
    { MachineId        = DiscoId.Create()
      ...
      WorkSpace        = filepath "/Disco"
      LogDirectory     = filepath "/Disco"
      CollectMetrics   = false
      MetricsHost      = IPv4Address "127.0.0.1"
      MetricsPort      = port 5000us
      MetricsDb        = "disco"
      AssetDirectory   = filepath "/Disco"
...
let clients =
  Seq.map
    (fun (service:DiscoMachine) ->
      makeClient service.MachineId (sprintf "%A Client" service.HostName |> name))
    machines
  |> List.ofSeq

let makeTree (machine:DiscoMachine) =
  let randomFileName () = DiscoId.Create().Prefix() |> filepath
