...
type State =
  ...
  static member DiscoveredService_ (id:DiscoId) =
  ...
  static member FromFB(fb: StateFB) : Either<DiscoError, State> =
    either {
      ...
      let! project =
        #if FABLE_COMPILER
        DiscoProject.FromFB fb.Project
        #else
        let pfb = fb.Project
        if pfb.HasValue then
          let projectish = pfb.Value
          DiscoProject.FromFB projectish
        else
      ...
      let! clients =
        ...
        Array.fold
          (fun (m: Either<DiscoError,int * Map<ClientId, DiscoClient>>) _ -> either {
            ...
            #if FABLE_COMPILER
            let! client = fb.Clients(i) |> DiscoClient.FromFB
            #else
            let! client =
...
module State =
  ...
  let sites = Optic.get (State.Project_ >-> DiscoProject.Config_ >-> DiscoConfig.Sites_)
  let activeSite = Optic.get (State.Project_ >-> DiscoProject.Config_ >-> DiscoConfig.ActiveSite_)
  ...
  let setSites = Optic.set (State.Project_ >-> DiscoProject.Config_ >-> DiscoConfig.Sites_)
  let setActiveSite = Optic.set (State.Project_ >-> DiscoProject.Config_ >-> DiscoConfig.ActiveSite_)
  ...
  let updateClient (client: DiscoClient) (state: State) =
    if Map.containsKey client.Id state.Clients then
      { state with Clients = Map.add client.Id client state.Clients }
    else
      state
  ...
  let removeClient (client: DiscoClient) (state: State) =
    { state with
  ...
  let updateMachine (machine: DiscoMachine) (state: State) =
  ...
  let updateProject (project: DiscoProject) (state: State) =
...
type StateMachine =
  ...
  | UpdateClient            of DiscoClient
  | RemoveClient            of DiscoClient
  ...
  static member FromFB (fb: StateMachineFB) =
    ...
    // | |___| | |  __/ | | | |_
    //  \____|_|_|\___|_| |_|\__|
    | x when x = StateMachinePayloadFB.DiscoClientFB ->
  ...
  member self.ToOffset(builder: FlatBufferBuilder) : Offset<StateMachineFB> =
    ...
    | AddClient       client ->
      ...
      StateMachineFB.StartStateMachineFB(builder)
      StateMachineFB.AddAction(builder, StateMachineActionFB.AddFB)
      StateMachineFB.AddPayloadType(builder, StateMachinePayloadFB.DiscoClientFB)
#if FABLE_COMPILER
