...
module Array =
  ...
  let inline replaceById< ^t when ^t : (member Id : DiscoId)> (newItem : ^t) (arr: ^t[]) =
    Array.map
      (fun (x: ^t) ->
        let newId = (^t : (member Id : DiscoId) newItem)
        let xId = (^t : (member Id : DiscoId) x)
        if newId = xId
        then newItem
        else x)
      arr
