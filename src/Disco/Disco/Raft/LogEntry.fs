...
type RaftLogEntry =
  ...
  static member ParseLogFB (fb: LogFB)
                           (sibling: Either<DiscoError,RaftLogEntry option>)
                           : Either<DiscoError,RaftLogEntry option> =
  ...
  static member FromFB (logs: LogFB array) : Either<DiscoError, RaftLogEntry option> =
...
module LogEntry =
  ...
  let make term data =
    LogEntry(DiscoId.Create(), index 0, term, data, None)
  ...
  let mkConfig term mems =
    Configuration(DiscoId.Create(), index 0, term, mems, None)
  ...
  let mkConfigChange term changes =
    JointConsensus(DiscoId.Create(), index 0, term, changes, None)
  ...
  let snapshot mems data log =
    ...
    in
      Snapshot(DiscoId.Create(),idx + 1<index>,term,idx,term,mems,data)
