...
module IrisService =
  ...
  let private broadcastMsg (state: IrisState) (_: Origin) (cmd: StateMachine) =
    cmd
  ...
  let private stateChanged (state: IrisState) (oldstate: RaftState) (newstate: RaftState) =
    newstate
  ...
  let private requestAppend (leader: Leader) (sm: StateMachine) =
    AppendEntry sm
    |> Binary.encode
    ...
    Tracing.trace (tag "forwardCommand") <| fun () ->
      ...
      | None ->
        ...
        | Some mem ->
          ...
          Option.iter (fun leader -> requestAppend leader sm) leader
  ...
  let private handleClientResponse (state: IrisState) (_: RawClientResponse) =
    state
  ...
  let private addMember (agent: IrisAgent) (mem: RaftMember) =
  ...
  let private removeMember (store: IAgentStore<IrisState>)
                           (agent: IrisAgent)
                           (id: Id) =
  ...
  let private append (agent: IrisAgent) (cmd: StateMachine) =
    (Origin.Service, cmd)
    |> IrisEvent.Append
    |> Msg.Event
    |> agent.Post
  ...
  let private makeService ctx iris store agent subscriptions cts =
    { new IIrisService with
        member self.Start() = start ctx iris store agent subscriptions
        ...
        member self.Append cmd = append agent cmd
