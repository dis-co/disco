...
type StateMachine =
  ...
  member sm.DispatchStrategy
    with get () =
      match sm with
      // Project
      | UpdateProject           _ -> Replicate
      | UnloadProject           _ -> Replicate
      // Member
      | AddMember               _ -> Replicate
      | UpdateMember            _ -> Replicate
      | RemoveMember            _ -> Replicate
      // Client
      | AddClient               _ -> Replicate
      | UpdateClient            _ -> Replicate
      | RemoveClient            _ -> Replicate
      // GROUP
      | AddPinGroup             _ -> Replicate
      | UpdatePinGroup          _ -> Replicate
      | RemovePinGroup          _ -> Replicate
      // PIN
      | AddPin                  _ -> Replicate
      | UpdatePin               _ -> Replicate
      | RemovePin               _ -> Replicate
      | UpdateSlices            _ -> Publish
      // CUE
      | AddCue                  _ -> Replicate
      ...
      | RemoveCueList           _ -> Replicate
      // User
      | AddUser                 _ -> Replicate
      | UpdateUser              _ -> Replicate
      | RemoveUser              _ -> Replicate
      // Session
      | AddSession              _ -> Replicate
      | UpdateSession           _ -> Replicate
      | RemoveSession           _ -> Replicate
      // Discovery
      | AddDiscoveredService    _ -> Replicate
      | UpdateDiscoveredService _ -> Replicate
      | RemoveDiscoveredService _ -> Replicate
      | UpdateClock             _ -> Publish
      | Command                 _ -> Replicate
      | DataSnapshot            _ -> Replicate
  ...
  member sm.PersistanceStrategy
    with get () =
      ...
      | RemoveMember            _ -> Durable
      // Client
      | AddClient               _ -> Durable
      | UpdateClient            _ -> Durable
      | RemoveClient            _ -> Durable
      // GROUP
      | AddPinGroup             _ -> Durable
      | UpdatePinGroup          _ -> Durable
      | RemovePinGroup          _ -> Durable
      // PIN
      | AddPin                  _ -> Durable
      | UpdatePin               _ -> Durable
      | RemovePin               _ -> Durable
      | UpdateSlices            _ -> Volatile
      // CUE
      | AddCue                  _ -> Durable
      | UpdateCue               _ -> Durable
      | RemoveCue               _ -> Durable
      | CallCue                 _ -> Volatile
      // CUE
      | AddCueList              _ -> Durable
      | UpdateCueList           _ -> Durable
      | RemoveCueList           _ -> Durable
      // User
      | AddUser                 _ -> Durable
      | UpdateUser              _ -> Durable
      | RemoveUser              _ -> Durable
      // Session
      | AddSession              _ -> Volatile
      | UpdateSession           _ -> Volatile
      | RemoveSession           _ -> Volatile
      // Discovery
      | AddDiscoveredService    _ -> Volatile
      | UpdateDiscoveredService _ -> Volatile
      | RemoveDiscoveredService _ -> Volatile
      | UpdateClock             _ -> Volatile
      | Command                 _ -> Durable
      | DataSnapshot            _ -> Durable
      | SetLogLevel             _ -> Volatile
      | LogMsg                  _ -> Volatile
