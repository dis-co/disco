...
public sealed class Patch : Table {
  ...
  public Iris.Types.IOBox GetIOBoxes(int j) { return GetIOBoxes(new Iris.Types.IOBox(), j); }
  public Iris.Types.IOBox GetIOBoxes(Iris.Types.IOBox obj, int j) { int o = __offset(8); return o != 0 ? obj.__init(__indirect(__vector(o) + j * 4), bb) : null; }
  ...
  public static VectorOffset CreateIOBoxesVector(FlatBufferBuilder builder, Offset<Iris.Types.IOBox>[] data) { builder.StartVector(4, data.Length, 4); for (int i = data.Length - 1; i >= 0; i--) builder.AddOffset(data[i].Value); return builder.EndVector(); }
