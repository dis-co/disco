...
module Monitor =
    ...
    let private buildEventDetails (ev,data) =
      match ev with
      // data is a reconnect interval
      | ev when ev = (uint16)ZMQ.EVENT_CONNECT_RETRIED -> ConnectRetried data
      // data is a socket handle (a.k.a. file descriptor, or fd)
      | ev when ev = (uint16)ZMQ.EVENT_LISTENING       -> Listening    data
      | ev when ev = (uint16)ZMQ.EVENT_CONNECTED       -> Connected    data
      | ev when ev = (uint16)ZMQ.EVENT_ACCEPTED        -> Accepted     data
      | ev when ev = (uint16)ZMQ.EVENT_CLOSED          -> Closed       data
      | ev when ev = (uint16)ZMQ.EVENT_DISCONNECTED    -> Disconnected data
      // data is a ZeroMQ error number (for use with ZMQError)
      | ev when ev = (uint16)ZMQ.EVENT_BIND_FAILED     -> BindFailed   (new ZError(data))
      | ev when ev = (uint16)ZMQ.EVENT_ACCEPT_FAILED   -> AcceptFailed (new ZError(data))
      | ev when ev = (uint16)ZMQ.EVENT_CLOSE_FAILED    -> CloseFailed  (new ZError(data))
      // data is meaningless
      | ev when ev = (uint16)ZMQ.EVENT_CONNECT_DELAYED -> ConnectDelayed
      | ev when ev = (uint16)ZMQ.EVENT_MONITOR_STOPPED -> MonitorStopped
      | _                                              -> Unknown
